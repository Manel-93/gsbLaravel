...
*/
	...
	public function getFichesValideesDuJour($date) {
		$req = "SELECT f.idVisiteur, v.nom, v.prenom, f.mois, f.montantValide, f.dateModif 
				FROM fichefrais f
				JOIN visiteur v ON f.idVisiteur = v.id
				WHERE f.dateModif = '$date' AND f.idEtat = 'VA'";
		
		$res = $this->monPdo->query($req);
		$fichesValidees = array();
		$laLigne = $res->fetch(PDO::FETCH_ASSOC);
		
		while($laLigne != null) {
			$fichesValidees[] = (object)[
